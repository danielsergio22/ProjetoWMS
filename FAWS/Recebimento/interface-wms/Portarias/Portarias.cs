    ...
    {     
        ...
        public FrmPortarias()
        {
            ...
            AbasFormatacoes.PassarCampos(btnBuscarPor, btnVertodosPort, btnNovoCadastroPortarias, btnSalvarAtualizarPort,
                                         btnEditarPort, btnAtvInatCadastroPort, btnCancelarPort, btnMenuPort);
            ...
            AbasFormatacoes.PassarCampos(tsmiNavegacaoPort, tsmiSistemaPort, tsmiUsuarioPort, tsmiAjudaPort, tsmiRegistrarPort, 
                                         ...
                                         tsmiSairPort, tsmiPreferenciasPort, tsmiVerMatriculaPort, tsmiSuportePort);
            ...
            AbasFormatacoes.PassarCamposCabecalho(lbNomeSistemaPort, lblDatahoraPort, lbUsuarioPort, lbNomeCompaniaPort, lbNomeUsuarioPort);
        ...
        {
            if (btnCancelarPort.Text == "Sair")
            {
                Application.Exit();
            }
            else if (btnCancelarPort.Text == "Cancelar")
            {
                btnCancelarPort.Text = "Sair";
                btnCancelarPort.Image = Interface_WMS_Recebimento.Properties.Resources.sair;
                TipDicas.SetToolTip(btnCancelarPort, "Clique para Sair");
                LimparTodosOsCampos();
            }   
        }
        ...
        private void btnAtvInatCadastroPort_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!ConexaoBD.VerificarDadoNaoExisteBD(connec, verifica))
                {
                    ...
                    if (!cod)
                    {
                        if (GerenciarAcessos.getUsuario.Equals("RECEBIMENTO@SUPERVISOR"))
                        {
        ...
        private void btnMenuPort_Click(object sender, EventArgs e)
        {
            this.Close();
        }
        ...
        {
            MessageBox.Show("Funcionalidade indisponível nesta versão!", "FAWS WMS", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
